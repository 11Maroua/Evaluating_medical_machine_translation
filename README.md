...
- **COMET** — a score produced by a neural model trained to predict translation
  ...
  ranges from 0 to 1.
- **MedTerm** — a terminology-aware metric we implement, grounded in a curated
...
├── src/
│   ├── medcon.py                      # Grouped MedTerm implementation
...
the MeSH+SNOMED dictionary to determine which best supports the MedTerm metric.
...
#### Notebook 01: MedTerm Evaluation on GPT-4o

We evaluated GPT-4o translations of 49 WMT24 biomedical abstracts using MedTerm,
BLEU, and COMET.

| Metric | Mean | Std | Min | Max |
|---|---|---|---|---|
| MedTerm F1 | 0.398 | 0.475 | 0.000 | 1.000 |
| BLEU | 49.15 | 12.64 | 18.33 | 79.77 |
| COMET | 0.875 | 0.034 | 0.710 | 0.914 |

**Correlations between metrics:**

| | MedTerm F1 | BLEU | COMET |
|---|---|---|---|
| MedTerm F1 | | r = +0.071 (p=0.626) | r = -0.029 (p=0.841) |
| BLEU | | | r = +0.408 (p=0.004) |

MedTerm is virtually uncorrelated with both BLEU (r=+0.071) and COMET (r=-0.029),
...
- **6 unique extras**: a French medical term from the dictionary appears in the
  ...
  content not present in the original text, though MedTerm matching alone is
...
|---|---|---|---|---|
| MedTerm F1 | +0.157 | 0.452 | +0.134 | 0.524 |
...
The binary terminology error flag (0/1) is correlated with MedTerm F1 using the
point-biserial correlation, and here are the results:

| Statistic | Value |
|---|---|
| n | 25 |
| Docs with terminology error | 21 / 25 |
| Point-biserial r | +0.179 |
| p-value | 0.391 |
| Mean MedTerm F1 (with error) | 0.462 |
| Mean MedTerm F1 (without error) | 0.250 |

> Non-significant result (p >= 0.05). The correlation is positive but the small size of the annotated subset (n=25) limits the detectability of any effect.
...
| Metric | Pearson r | p-val | Spearman rho | p-val |
|---|---|---|---|---|
| MedTerm F1 | -0.045 | 0.760 | -0.046 | 0.755 |
| BLEU | -0.019 | 0.895 | -0.029 | 0.842 |

Neither MedTerm nor BLEU correlates with MedGemma scores (all p > 0.05).
...
though the direction is positive (r=+0.298) and close to significance. The score
distribution is highly concentrated: MedGemma assigns 4/5 to 24 out of 25 documents
and 5/5 to only 1, while the physician assigned 4/5 to 17 and 5/5 to 8. Both
evaluators agree on the general quality level but MedGemma is less willing to assign
the highest score.
...
nearly identical translations. The one document where RAG degrades MedTerm most
(doc #12, delta=-0.200) is a COVID-19 vaccination study where the RAG translation
introduces a `**Resume :**` header not present in the source, a formatting artifact
...
MedTerm rely on exact matching and are insensitive to subtle differences in
